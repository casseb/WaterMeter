...
public abstract class Dialog {
	 ...
	 */
	private Arquivo boxFileObject;

	/**
	 * Instancia de BoxFolderObject para uso interno
	 */
	private Pasta boxFolderObject;

	/**
	 * Instância dos dados customizados de acesso
	 */
	private Acesso access = new Acesso();
	...
	// Construtor
	public Dialog(TelegramBot bot, Usuario person, Rota route, String message) {
		super();
		this.bot = bot;
		this.person = person;
		this.route = route;
	...
	protected Dialog finishStep(boolean menu) {
		...
		model.addMessageLog(new LogMensagem(person, answer.toString(), "Enviado pelo Bot"));
	...
	protected void iNeedAFile(String... folder) {
		...
		for (String string : folder) {
			...
			if (currentFolder == null) {
				currentFolder = new Pasta(string, model, rootFolder);
			}
	 ...
	 */
	protected void sendFiles(File file, List<Usuario> persons) {
		for (Usuario person : persons) {
			bot.execute(new SendDocument(person.getIdTelegram(), file));
		}
	}
	...
	protected File createPDF(String fileName, List<Chunk> chunks, String... folder) {
		...
		model.addBoxFileObject(new Arquivo(fileName+".pdf",model,file,currentFolder));
	 ...
	 */
	protected void answer(Usuario person, String message) {
	 ...
	 * @param trigger
	 */
	protected void answer(Usuario person, String message, LocalDateTime trigger) {
		MensagemAgendada scheduleMessage = new MensagemAgendada(person, message, trigger);
	 ...
	 */
	protected void sendMessages(String message, List<Usuario> persons) {
		for (Usuario person : persons) {
	...
	public void setBoxFile(String fileId, String name) {
		...
		this.boxFileObject = new Arquivo(name, bot.getFullFilePath(file), model, boxFolderObject);
	}
